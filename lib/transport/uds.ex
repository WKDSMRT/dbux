defmodule DBux.Transport.UDS do
  require Logger
  use Connection
  ...
  def start_link(parent, options) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Start link")
    Connection.start_link(__MODULE__, {parent, options})
  end


  def do_connect(transport_proc) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Do connect: #{inspect(transport_proc)}")
    Connection.call(transport_proc, :connect)
  end


  def do_disconnect(transport_proc) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Do disconnect: #{inspect(transport_proc)}")
    Connection.call(transport_proc, :disconnect)
  end


  def do_send(transport_proc, data) when is_binary(data) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Do send: #{inspect(transport_proc)}")
    Connection.call(transport_proc, {:send, data})
  end


  def do_begin(transport_proc) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Do begin: #{inspect(transport_proc)}")
  ...
  def init({parent, options}) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Init")
    {:ok, %{ parent: parent, sock: nil, state: :handshake, path: options[:path]}}
  end
  ...
  def connect(_, %{path: path} = state) do
    ...
    case :gen_tcp.connect({:local, path}, 0, [active: false, mode: :binary, reuseaddr: true], @connect_timeout) do
      ...
      {:error, reason} ->
        Logger.warn("[DBux.Transport.UDS #{inspect(self())}] Connect: Failed to connect to #{path}: #{inspect(reason)}")
  ...
  def disconnect(_, %{sock: sock} = state) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Disconnect")
  ...
  def handle_call(:connect, _sender, %{sock: nil} = state) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Handle call: Connect while disconnected")
  ...
  def handle_call(:connect, _sender, state) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Handle call: Connect while connected")
  ...
  def handle_call(:disconnect, _sender, %{sock: nil} = state) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Handle call: Disconnect while disconnected")
    {:reply, {:error, :already_connected}, state}
  end
  ...
  def handle_call(:disconnect, _sender, state) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Handle call: Disconnect while connected")
  ...
  def handle_call({:send, _}, _sender, %{sock: nil} = state) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Handle call: Send while disconnected")
  ...
  def handle_call({:send, data}, _sender, %{sock: sock} = state) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Handle call: Send while connected")
    case :gen_tcp.send(sock, data) do
      :ok ->
        {:reply, :ok, state}

      {:error, reason} ->
        Logger.warn("[DBux.Transport.UDS #{inspect(self())}] Failed to send data: #{inspect(reason)}")
  ...
  def handle_call(:begin, _sender, %{sock: nil} = state) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Handle call: Begin while disconnected")
    {:reply, {:error, :not_connected}, state}
  end
  ...
  def handle_call(:begin, _sender, %{sock: sock} = state) do
    if @debug, do: Logger.debug("[DBux.Transport.UDS #{inspect(self())}] Handle call: Begin while connected")
    case :gen_tcp.send(sock, "BEGIN\r\n") do
      ...
      {:error, reason} ->
        Logger.warn("[DBux.Transport.UDS #{inspect(self())}] Failed to begin: #{inspect(reason)}")
        {:reply, {:error, reason}, state}
    end
  end
  ...
  def handle_info({:tcp_closed, _}, %{parent: parent} = state) do
    Logger.warn "[DBux.Transport.UDS #{inspect(self())}] TCP connection closed"
  ...
  def handle_info({:tcp, _sock, "OK " <> _}, %{state: :handshake, parent: parent} = state) do
    if @debug, do: Logger.debug "[DBux.Transport.UDS #{inspect(self())}] Authentication succeeded"
  ...
  def handle_info({:tcp, _sock, "ERROR " <> reason}, %{state: :handshake, parent: parent} = state) do
    Logger.warn "[DBux.Transport.UDS #{inspect(self())}] Authentication error: #{reason}"
  ...
  def handle_info({:tcp, _sock, "REJECTED " <> _, reason}, %{state: :handshake, parent: parent} = state) do
    Logger.warn "[DBux.Transport.UDS #{inspect(self())}] Authentication failed: #{reason}"
  ...
  def handle_info({:tcp, _, data}, %{state: :ready, parent: parent} = state) do
    if @debug, do: Logger.debug "[DBux.Transport.UDS #{inspect(self())}] TCP read: #{inspect(data)}"
